...
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #makes it enter by row instead of column
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "quatro", "cinco"), 
                                  c("un", "deux", "trois", "quatre"))) #rows in spanish, columns in french

matrix1[4, ] #
matrix1[ , 3] #all rows of data in column 3
matrix1[c(12, 14)] #12th and 14th item
matrix1[c(12:14)] #12th thru 14th item
matrix1[2:4, 1:3] #

#building matrix with separate elements
cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
...
list1[[2]] #referring to list items, must use double brackets
list1["name"]

# Data Frames ---- can think about like an excel spreadsheet
d <- c(1, 2, 3, 4) # What type of vector?
e <- c("red", "white", "red", NA) # What type of vector?
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector?
dataframe1 <- data.frame(d,e,f) # can put them together because they're all the same length
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # update names through names function

dataframe1[1:2,] # rows 1 and 2, all columns
dataframe1[1:2,"Color"]
dataframe1[1:2,3]
...
```{r DataExercise-1a}
high_F <- c(64,71,72,63,51,52,61,64,67,73)
low_F <- c(42,49,60,46,32,32,41,47,52,54)
```
...
```{r DataExercise-1b}
high_celcius <- (high_F-32)*5/9;high_celcius
low_celcius <- (low_F-32)*5/9;low_celcius
...
```{r DataExercise-1c}
#make new data frame
temp_10_days <- data.frame(high_F,low_F,high_celcius,low_celcius)

# update names through names function
names(temp_10_days) <- c("Fahrenheit High","Fahrenheit Low","Celcius High", "Celcius Low"); View(temp_10_days) 
...
today <- Sys.Date()
format(today, format = "%B") #return long name of month
#format(today, format = "")
#format(today, format = "")
#format(today, format = "")
...
#Since the format is year-month-day we will use function ymd() - only need to tell it the order when using lubridate
...
ifelse(year(date_obj_past)%%100 > 23,
       year(date_obj_past)-100,
       year(date_obj_past))
