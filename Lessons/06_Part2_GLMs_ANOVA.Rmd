...
For example: Does total litter biomass differ among sites at the Niwot Ridge LTER? - is mean different at different sites?
```{r}
# Wrangle the data
...
#if p is less than .05, reject null hypothesis
...
# Post-hoc test - want to figure out which means are not the same
# TukeyHSD() computes Tukey Honest Significant Differences
TukeyHSD(Litter.Totals.anova)

# Extract groupings for pairwise relationships - treatments with same letter are not sig. different - so tells which groups are the same
...
#reject null means mean across groups is different
...
```{r}
# Format as aov - using * instead of +
...
#compare means for each one of levels - use litter interaction levels
#interaction of needles and evergreen forest cannot be compared with any other interaction
