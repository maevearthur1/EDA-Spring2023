...
#same plot but in different way - good to store in object and then print that object
...
# Separate plot with facets - creating one plot for each site
PMplot.faceted <-
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + #adding trend line (lm=linear model)
  geom_abline(aes(slope = 16, intercept = 0)) #can use this when you know slope and intercept
print(NvsP)
...
# Violin plot #also shows you density #wider spreads
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# Using a gathered (pivoted) dataset - better
Nutrientplot7 <-   
...
# Ridgeline plot - will show density of distribution over time - used fill to fiill area below curve of plot
Nutrientplot6 <-
