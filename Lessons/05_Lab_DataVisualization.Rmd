...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    x=0,
    y=0.025,
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6')) #paste command brings together strings and inserts spaces, expression command used for mathematical expressions 
    )
...
PeterPaul.chem.nutrients <- read.csv(
  here(processed_data,"NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv"), #here package to find a path
...
#Convert month to a factor -- with 12 levels, labelled with month names - want levels (categories)
...
ggplot(PeterPaul.chem.nutrients,
  ...
  geom_bar() +
  scale_x_discrete(drop=FALSE) + #x axis is not continuous anymore, and we want to show empty categories
  labs(
    title="My Plot",
    x = "Month"
  )
...
PPLake <- PeterPaul.chem.nutrients %>%
  filter(depth==0) %>%
  ggplot(aes(
    x=daynum, 
    y=temperature_C, 
