...
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #every time lakename matches an element in this vector R will select it

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304)) #only days (daynum) 152-304
```
...
NTL.phys.data.ex4<-NTL.phys.data.ex3 %>%
  filter(depth==0) %>%
  drop_na(depth) %>%
  summarize(mean_suface_temp=mean(temperature_C))
...
NTL.phys.data.ex5<- pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value")
