...
#can see seasonality in the rainfall, can't really see trend
ggplot(BR_complete, aes(y = Rainfall, x = Date)) +
  geom_line() +
  geom_smooth(method = lm)

#can see seasonality in the temperature, see clear increasing trend
...
#rainfall ts
BR_ts_rainfall <- ts(BR_complete$Rainfall,
                   start=c(f_year,f_month),
                   frequency=12)

#temperature ts
BR_ts_temp <- ts(BR_complete$Temperature_C,
                   start=c(f_year,f_month),
                   frequency=12) 
...
#rainfall decomposed - the bigger the bar, the smaller the contribution to your overall series - trend does not seem to be significant
BR_rainfall_decomp <- stl(BR_ts_rainfall, s.window = "periodic")
plot(BR_rainfall_decomp)

#temperature decomposed - seasonality and trend are playing similar parts - see increasing trend around 1980s (in trend)
BR_temp_decomp <- stl(BR_ts_temp, s.window = "periodic")
plot(BR_temp_decomp)
...
BR_rainfall_df <- as.data.frame(BR_rainfall_decomp$time.series[,1:3])
BR_temp_df <- as.data.frame(BR_temp_decomp$time.series[,1:3])

BR_rainfall_df <- mutate(BR_rainfall_df,
        Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date)

BR_temp_df <- mutate(BR_temp_df,
...
#rainfall
ggplot(BR_rainfall_df, aes(x=Date)) +
  geom_line(aes(y = Observed), col = "black") +
  geom_line(aes(y = trend), col = "red") +
  ylab("Rainfall")

#temperature
ggplot(BR_temp_df, aes(x=Date)) +
  geom_line(aes(y = Observed), col = "black") +
  geom_line(aes(y = trend), col = "red") +
  ylab("Temperature")
...
#rainfall - no trend
rain_trend <- trend::smk.test(BR_ts_rainfall)
summary(rain_trend)

#temperature - 
temp_trend <- trend::smk.test(BR_ts_temp)
summary(temp_trend)


# Exercise 12: Now remove the seasonality and apply another test. Compare the results with what you obtained on Exercise 9.

#take observed, subtract seasonal, provide that series to original mann-kendall
